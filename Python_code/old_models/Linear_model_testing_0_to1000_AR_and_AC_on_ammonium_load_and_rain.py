...
extra_valid_data = extra_valid_data.loc[:, ["ammonium_load_AN_kg_h"]]
...
AR_AL_broaders = range(1,100,1)
AR_R_broaders = range(1,100,1)
AC_AL_broaders = range(1,100,1)
AC_R_broaders = range(1,100,1)
...
for AR_AL_lags in AR_AL_broaders:
    for AR_R_lags in AR_R_broaders:
        for AC_AL_accumulator in AC_AL_broaders:
        #Print modelling progress
            print(f"AR_AL terms: {AR_AL_lags}, AR_R terms: {AR_R_lags}, AC_AL terms: {AC_AL_accumulator}")
...
preformace_table.to_csv("AR_0_to_480_lagged_values_ammonium_load.csv")        
