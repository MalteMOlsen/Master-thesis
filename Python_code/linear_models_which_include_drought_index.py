...
for i in range(1,49,1):  
    ...
    for lag in [48,48*2,48*3,48*3,48*4,48*5,48*6,48*7]:
            #Print modelling progress
            print(f"AR terms: {lag}")
            
            #Lagging or accumulating the chosen column
            X_training.loc[:,lag_feature_three+"_lag_"+str(lag)] = X_training[lag_feature_three].shift(lag)  
            X_valid.loc[:,lag_feature_three+"_lag_"+str(lag)] = X_valid[lag_feature_three].shift(lag) 
...
preformace_table.to_csv("AR_375_ammonium_and_AR_350_rain_with_1_weeks_lag_of_drouht_linear_model_accuracy_all_forecasting_horizon.csv")
