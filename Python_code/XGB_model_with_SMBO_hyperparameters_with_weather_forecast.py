...
for i in range(1,49,1):  
    ...
    data_dir_path_forecast_X_data = Path(__file__).parent/ "forecasting_data"
    
    X_training_path = str(forecasting_horizon)+"X_training_with_forecast.csv"

    X_training = pd.read_csv(data_dir_path_forecast_X_data / X_training_path)

    X_training=X_training.drop("Unnamed: 0",axis=1)


    #Load validation data
    X_valid_path = str(forecasting_horizon)+"X_valid_with_forecast.csv"

    X_valid = pd.read_csv(data_dir_path_forecast_X_data / X_valid_path)
    ...
    target_valid_load=target_valid_load[:-forecasting_horizon]

    target_valid_seasonality_pred_path = str(forecasting_horizon)+"seasonal_adjusted_df_valid.csv"
    ...
    target_training_resid=target_training_resid[:-forecasting_horizon]

    data_dir_path_2 = Path(__file__).parent / "hyperparameters_to_AN_XGB"
...
preformace_table.to_csv("XGB_with_AN_tank_hyperparameters_optimasation_with_weather_forecast.csv")
