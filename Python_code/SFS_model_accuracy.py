...
for i in range(1,49,1): 
    ...
    #Select columns that are in the SFS file
    
    #load the file
    
    data_dir_path_for_SFS = Path(__file__).parent / "SFS_files"
    sfs_file_name = "Sequential_feature_selection_forecasting_horizon_"+str(forecasting_horizon)+"_with_50_numbers_of_parameters.csv"
    sfs_list_of_feature = pd.read_csv(data_dir_path_for_SFS / sfs_file_name)
    
    sfs_list_of_feature = sfs_list_of_feature.loc[:, "Feature"]
    sfs_list_of_feature=sfs_list_of_feature.tolist()
    

    X_training = X_training[X_training.columns.intersection(sfs_list_of_feature)]
    X_valid = X_valid[X_valid.columns.intersection(sfs_list_of_feature)]
...
preformace_table.to_csv("SFS_optimal_model_performance.csv")
